...
func itsdangerousResign(c *Cookie, data string, secret []byte) string {
	parsedData := c.parsedDataFor(itsdangerousDecoder).(*itsdangerousParsedData)
	algorithm := parsedData.algorithm
	var toBeSigned []byte
	var cookieReturn string
	if len(parsedData.decodedTimestamp) > 0 {
		toBeSigned = []byte(data + itsdangerousSeparator + parsedData.timestamp)
		cookieReturn = data + itsdangerousSeparator + parsedData.timestamp
	} else {
		toBeSigned = []byte(data)
		cookieReturn = data
	}

	if parsedData.compressed {
		toBeSigned = append([]byte("."), toBeSigned...)
		cookieReturn = "." + cookieReturn
	}

	sig := itsdangerousCompute(algorithm, secret, toBeSigned)
	cookieReturn = cookieReturn + itsdangerousSeparator + string(sig)
	return cookieReturn
}
