...
func (c *Cookie) Unsign(wl *Wordlist, concurrencyLimit uint64) (key []byte, success bool) {
	...
	shouldUseYii := c.hasParsedDataFor(yiiDecoder)

	// This looks a bit silly right now, but as we add more decoders, this
	// should be here to ensure we don't do pointless work.
	if !shouldUseDjango && !shouldUseFlask && !shouldUseJwt && !shouldUseRack && !shouldUseExpress && !shouldUseLaravel && !shouldUseItsDangerous && !shouldUseYii {
		return nil, false
	}
	...
	for _, entry := range wl.Entries() {
		...
		go func(entry []byte) {
			...
			if shouldUseYii && yiiUnsign(c, entry) {
				c.wasUnsignedBy(yiiDecoder, entry)
			}
		}(entry)
	}
...
func (c *Cookie) Resign(data string) string {
	...
	case expressDecoder:
		return expressResign(c, data, c.unsignedKey)
	case laravelDecoder:
		return laravelResign(c, data, c.unsignedKey)
	case jwtDecoder:
		return jwtResign(c, data, c.unsignedKey)
	case rackDecoder:
		return rackResign(c, data, c.unsignedKey)
	case itsdangerousDecoder:
		return itsdangerousResign(c, data, c.unsignedKey)
	case yiiDecoder:
		return yiiResign(c, data, c.unsignedKey)
	default:
		return ""
	}
}
